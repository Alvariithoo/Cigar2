(function () {
    ...
    */
    ...
    function drawPosition() {
        ...
        let beginX = view.width / camera.viewportScale - width;
        let beginY = view.height / camera.viewportScale - height;
    ...
    function drawGrid() {
        ...
        const step = 50;
        const cW = view.width / camera.scale;
        const cH = view.height / camera.scale;
    ...
    function drawMinimap() {
        ...
        const beginX = view.width - width;
        const beginY = view.height - height;
    ...
    function drawGame() {
        ...
        /* if (settings.jellyPhysics) {
         ...
         mainCtx.fillRect(0, 0, view.width, view.height);
         ...
         if (leaderboard.visible) {
             mainCtx.drawImage(
                 leaderboard.canvas,
                 view.width / camera.viewportScale - 10 - leaderboard.canvas.width,
                 10);
         }
         if (settings.showChat && (chat.visible || isTyping)) {
             ...
             mainCtx.drawImage(
                 ...
                 (view.height - 55) / camera.viewportScale - chat.canvas.height
             );
             mainCtx.globalAlpha = 1;
         }
         drawMinimap();
         ...
         if (minionControlled) {
             ...
             mainCtx.fillText(text, view.width / 2, 5);
             mainCtx.restore();
         }
         */
        //cacheCleanup();
    ...
    function cameraUpdate() {
        ...
        toCamera(application.stage);
        fromCamera(application.stage);
    }
    ...
    class Cell {
        ...
        update(relativeTime) {
            ...
            this.x = this.entity.position.x = this.ox + (this.nx - this.ox) * dt;
            ...
            if (this.destroyed) {
                this.entity.alpha = Math.max(120 - Date.now() + this.dead, 0) / 120;
            } else {
                this.entity.alpha = Math.min(Date.now() - this.born, 120) / 120;
            }
    ...
    function generateTextures() {
        ...
        cell.beginFill(0xffffff); // white color.
